...
class MCClipImageViewController: UIViewController {
    ...
    public var isRound = true
    
    
    // 裁剪的目标图片
    private var targetImage : UIImage = UIImage()
    // 裁剪的区域
    private var cropSize : CGSize = CGSize.init(width: UIScreen.main.bounds.size.width, height: UIScreen.main.bounds.size.width)
    // 裁切框的frame
    ...
    init(/*store: AssetStore*/ asset: PHAsset) {
        ...
        imageManager.requestImage(for: asset, targetSize: UIScreen.main.bounds.size , contentMode: .aspectFill, options: options) { [weak self](image, _) in
            guard let image = image else { return }
            self?.settingUIDataWithImage(image)
        }
    }
    ...
    func settingUIDataWithImage(_ image: UIImage,size:CGSize = CGSize.init(width: UIScreen.main.bounds.size.width, height: UIScreen.main.bounds.size.width)) {
        targetImage = image
        cropSize = size
    ...
    lazy var cancelButton: UIButton = {
        ...
        button.setImage(UIImage(named: "ClipImage_cancel"), for: .normal)
    ...
    lazy var rotatingButton: UIButton = {
        let button = UIButton.init(type: UIButton.ButtonType.custom)
        button.setImage(UIImage(named: "ClipImage_rotating"), for: .normal)
    ...
    lazy var sureButton: UIButton = {
        let button = UIButton.init(type: UIButton.ButtonType.custom)
        button.setImage(UIImage(named: "ClipImage_sure"), for: .normal)
